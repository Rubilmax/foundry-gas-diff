...
const trend = (value) => value > 0 ? ":x:" : value < 0 ? ":heavy_check_mark:" : ":heavy_minus_sign:";
...
function formatDiffMarkdown(rows) {
    const COLS = [
        { txt: "" },
...
if (options.help) {
    ...
    if (options.src.length < 2)
        throw Error("At least two different gas reports must be specfied");
